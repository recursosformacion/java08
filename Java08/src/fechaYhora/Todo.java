...
public class Todo {
	...
	public static void main(String[] args) {
		...
		LocalDate date2 = LocalDate.of(2015, Month.FEBRUARY, 32); // throws exception because the day is invalid
			...
			System.out.println("Lanza exepcion");
		}
		...
		// lDate1 = lDate1.plusMinutes(1); //C.E. plusMinutes method not applicable on
		// date.

		// Period

		Period yearAndWeek = Period.ofYears(1).ofWeeks(1); // chaining has no effect here. only the last method takes
															// effect.
		System.out.println(yearAndWeek); // output is P7D,

		yearAndWeek = Period.of(1, 0, 7);
		System.out.println("periodo:" + yearAndWeek); // Output is P1Y7D
		...
		Year y = Year.of(2014);
		System.out.println(ld1.adjustInto(y.atDay(1))); // adjust into method will have the same date as "this" in this
														// case this is ld1
		...
		// similarly we have methods for LocalDteTime, taking into all possible
		// combinations from LocalDate and LocalTime

		Set<String> zoneIds = ZoneId.getAvailableZoneIds(); // returns a modifiable copy of the set of zone IDs, not
															// null
		 ...
		 * unchanged.The months unit is adjusted to have an absolute value less than
		 * 11,with the years unit being adjusted to compensate. For example, a period
		 * of"1 Year and 15 months" will be normalized to "2 years and 3 months".
		 * 
		 */
		...
		// instant = instant.plus(20,ChronoUnit.MONTHS); //throws R.T.E
		// java.time.temporal.UnsupportedTemporalTypeException: Unsupported unit: Months

		// instant = instant.plus(20,ChronoUnit.YEARS); //throws R.T.E
		...
		// only the ZonedDateTIme class with the Period class can correctly modify time
		// data when dealing with daylight savings time.
		// when you need to calculate across daylight savings times, then using regular
		// plus or minus methods will not return a correct result.
		...
		// System.out.println(lDate2.format(df));//throws runtime exception because
		// LaocalDate does not have time, and the formatter expects time component as
		// well

		LocalDateTime ldt = LocalDateTime.now();
		// System.out.println(ldt.format(df)); //this too throws run time exception
		// because the date formatter expects offsetSeconds
